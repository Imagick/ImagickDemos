...
function getUrl(string $fullURL)
{
...
function borkBody($body, $classname, $method): \ManualEntry
{
...
function fixupXMLForParsing($body)
{
    $SR = [
        ...
        '&return.falseforfailure' => ' or &false; on failure',
        '&return.falseforfailure.style.procedural' => '&style.procedural; returns &false; on failure.',
        '&null;' => 'null',
...
//    file_put_contents("debug.xml", $body);

    return str_replace(array_keys($SR), array_values($SR), $body);
}


function getManualWords($classname, $method): ?\ManualEntry {

    $baseURL = "https://raw.githubusercontent.com/php/doc-en/master/reference/imagick/%s/%s.xml";
...
//    echo "$fullURL \n";
    ...
    $xml = fixupXMLForParsing($body);

    $manualEntry = borkBody($xml, $classname, $method);
    return $manualEntry;
}
